...
from utils import DATA_FOLDER_PATH, CLUSTER_FOLDER_PATH, FINETUNE_MODEL_PATH, evaluate_predictions, \
...
def evaluate(dataset, stage, suffix=None):
    ...
    if stage == "Rep":
        with open(os.path.join(CLUSTER_FOLDER_PATH, dataset, f"document_repr_lm-{suffix}.pk"), "rb") as f:
        ...
        with open(os.path.join(CLUSTER_FOLDER_PATH, dataset, f"data.{suffix}.pk"), "rb") as f:
            dictionary = pickle.load(f)
